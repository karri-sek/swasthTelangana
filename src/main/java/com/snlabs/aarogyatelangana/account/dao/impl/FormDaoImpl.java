...
public class FormDaoImpl implements FormDao {
	...
	public boolean updateInvasiveDetails(Invasive invasive) {
		...
		updateInvasiveRecord
				.append("UPDATE " + AppConstants.INVASIVE_PROCEDURES
						+ " SET F_DOCTOR_NAME = ?,")
				.append(" F_DIAGNOSIS_BASICS = ?,")
				.append(" F_DIAGNOSIS_INDICATION  = ?,")
				.append(" F_ADVANCED_MATERNAL_AGE = ?,")
				.append(" F_GENETIC_DISEASE = ?,")
				.append(" F_CARRIED_INVASIVE_PROCEDURE = ?,")
				.append(" F_COMPILICATION = ?,")
				.append(" F_ADDITIONAL_TEST = ?,")
				.append(" F_PROCEDURE_RESULT = ?,")
				.append(" F_PROCEDURE_CARRIED_DATE = ?,")
		...
		Object[] args = { invasive.getDoctorName(),
				...
				invasive.getGeneticDisease(), invasive.getProcedureResult(),
				invasive.getProcedureCarriedDate(),
				invasive.getMtpIndication(), invasive.getPatientID() };
		try {
			return jdbcTemplate.update(updateInvasiveRecord.toString(), args) > 0 ? true
					: false;
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public NonInvasive saveNonInvasive(NonInvasive nonInvasive) {
		...
		sb.append("INSERT INTO " + AppConstants.NON_INVASIVE_PROCEDURES + "(")
				...
				.append(")VALUES(?,?,?,?,?,?,?,?,?,?,?)");
		Object[] args = new Object[] { nonInvasive.getPatientID(),
				nonInvasive.getPatientName(), nonInvasive.getDoctorName(),
				nonInvasive.getDiagnosisIndication(),
				nonInvasive.getProcedures() == null ? null : nonInvasive.getProcedures().getUltrasound(),
				nonInvasive.getDeclarationDate(),
				nonInvasive.getProcedureCarriedDate(),
				nonInvasive.getOtherDiagnoseDetails(),
	...
	private int saveDiagnoseDetails(NonInvasive nonInvasive) {
		...
		try {
			insertPatientAddress
					...
					.append("F_RESEARCH_SCIENTIFIC) ")
					.append("VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
			Object[] args = { nonInvasive.getPatientID(),
					...
					nonInvasive.getDiagnoseDetails().getAssessmentOfCervical(),
					nonInvasive.getDiagnoseDetails().getUterineSizeAmenorrhea(),
					nonInvasive.getDiagnoseDetails().getSuspectedAdenexal(),
					nonInvasive.getDiagnoseDetails().getChromosomalAbnormalities(),
					...
					nonInvasive.getDiagnoseDetails().getTerminationOfPregnancy(),
					nonInvasive.getDiagnoseDetails().getChorionicVillus(),
					nonInvasive.getDiagnoseDetails().getIntraPartumEvents(),
					nonInvasive.getDiagnoseDetails().getComplicatingPregnancy(),
					nonInvasive.getDiagnoseDetails().getResearchScientific() };
			result = jdbcTemplate.update(insertPatientAddress.toString(), args);
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public Invasive saveInvasive(Invasive invasive) {
		...
		try {
			...
			invasive.getConveyDetails().setPatientID(
		...
		if (invasiveResult > 0 && diagnosisBasisResult > 0
				...
				&& invasiveProceduresResult > 0 && additionalTestsResult > 0 && conveyResult > 0) {
			return invasive;
		} else {
			return null;
		}
	...
	public Declaration saveDeclaration(Declaration declaration) {
		...
		return jdbcTemplate.update(sb.toString(), args) > 0 ? declaration
				: null;
	}
	...
	public NonInvasive getNonInvasiveDetails(long patientID) {
		...
		try {
			@SuppressWarnings("unchecked")
			List<NonInvasive> nonInvasiveList = (List<NonInvasive>)jdbcTemplate.queryForObject(
					nonInvasiveDetailsQuery.toString(), args,
	...
	public Invasive getInvasiveDetails(long patientID) {
		...
		invasiveDetailsQuery
				.append("SELECT * FROM " + AppConstants.INVASIVE_PROCEDURES
						+ " invasive").append(" WHERE invasive.F_PATIENT_ID=")
				.append(patientID);
		final Invasive invasive = new Invasive();
		try {
			jdbcTemplate.queryForObject(invasiveDetailsQuery.toString(),
					new RowMapper<Invasive>() {
						@Override
						public Invasive mapRow(ResultSet resultSet,
								int rowNumber) throws SQLException {
							invasive.setPatientID(resultSet
									.getInt("F_PATIENT_ID"));
							invasive.setDoctorName(resultSet
									.getString("F_DOCTOR_NAME"));

							// invasive.setD(resultSet.getString("F_DIAGNOSIS_INDICATION"));
							invasive.setAdvancedMaternalAge(resultSet
									.getString("F_ADVANCED_MATERNAL_AGE"));
							invasive.setGeneticDisease(resultSet
									.getString("F_GENETIC_DISEASE"));

							invasive.setProcedureResult(resultSet
									.getString("F_PROCEDURE_RESULT"));
							invasive.setProcedureCarriedDate(resultSet
									.getDate("F_PROCEDURE_CARRIED_DATE"));
							invasive.setMtpIndication(resultSet
